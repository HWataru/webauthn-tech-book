...
一般的なパスワード認証は、ユーザが入力したパスワードをサーバーに送信し、サーバーでは保存されたパスワード（一般的にはハッシュ化されている）と比較して認証を行います。
...
ざっくりと、U2FではYubiKeyのような外部デバイスが、
UAFではスマートフォンがAuthenticatorとして動作し、
WebAuthnではどちらも使えると捉えていただければよいかと思います。

秘密鍵は Authenticator から外部に漏れないように、セキュアチップ上などに保存されます。

#@# 秘密鍵はセキュリティチップ上に保存するとは限らない？

=== origin に紐づく認証情報の管理

Authenticator は、origin ごとのキーペアの生成と Challenge への署名を行います。
